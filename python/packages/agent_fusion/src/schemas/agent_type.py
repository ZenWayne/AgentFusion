...
class TypedCodeAgent(CodeAgent):
    """带有类型标识的 CodeAgent
    
    Wrapper around CodeAgent that adds a computed type field
    for Pydantic discrimination and type safety.
    """
    
    @computed_field
    @property
    def type(self) -> Literal[AgentTypeEnum.CODE_AGENT]:
        return AgentTypeEnum.CODE_AGENT
